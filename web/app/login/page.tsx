...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!password) {
            setFieldErrors({ password: t('login.password_required') });
            toastError(t('login.password_required'));
            setLoading(false);
            return;
        }

        const result = await login(email, password, churchSlug);
            ...
            <div className="max-w-md w-full space-y-8">
                <div>
                    ...
                    {churchName && (
                        <div className="mt-4 text-center">
                            <p className="text-sm text-gray-600">{t('login.entering')}</p>
                ...
                <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                    ...
                    <div className="text-center">
                        <Link href={`/register?church=${encodeURIComponent(churchSlug)}`} className="text-blue-600 hover:text-blue-500">
                            {t('ui.register')}
                        </Link>
                    </div>
                </form>
